...
// Clean category: ((Classe 2)) -> (C2)
const cleanCategory = (cat) => {
  if (!cat) return '';
  return cat
    .replace(/\(\(/g, '(')
    .replace(/\)\)/g, ')')
    .replace(/Classe\s*(\d)/gi, 'C$1');
};
  ...
  try {
    ...
    for (const [ownerName, ownerEntries] of Object.entries(byOwner)) {
      ...
      for (const entry of ownerEntries) {
        const horseLine = '   ' + entry.horseName.toUpperCase() + '\n';
    ...
    for (const [ownerName, ownerEntries] of Object.entries(byOwner)) {
      ...
      for (const entry of ownerEntries) {
        ...
        // Bold horse name (after 3 spaces)
        formatRequests.push({
          updateTextStyle: {
...
(async () => {
  ...
  if (auth && SPREADSHEET_ID) {
    ...
    for (const r of newRows) {
      ...
      const notes = `${r.date} — ${r.track} — ${r.race} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${cleanedStatus}`;
      
      sheetRows.push([
        currentDate,
        cleanedName,
        owner,
        sheetHyperlink('Engagement', r.raceUrl),
        notes
      ]);
    ...
    for (const r of changedRows) {
      const cleanedName = cleanHorseNameForSheet(r.horse);
      const owner = await lookupOwner(sheets, r.horse);
      const oldStatus = cleanStatus(r.oldStatut);
      const newStatus = cleanStatus(r.statut);
      const notes = `${r.date} — ${r.track} — ${r.race} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${oldStatus} → ${newStatus}`;
    ...
    // Process Partants (DP-P) - only once per horse/race
    for (const r of allCurrentDPP) {
      ...
      if (!postedPartants.has(partantKey)) {
        ...
        sheetRows.push([
          ...
          cleanedName,
          owner,
          sheetHyperlink('Partant', r.raceUrl),
          notes
        ]);
  ...
  const linesDPP = declaredParticipants.map(
    r => `• ${formatLink(cleanHorseNameForDiscord(r.horse), r.horseUrl)} — ${r.date} — ${r.track} — ${formatLink(r.race, r.raceUrl)} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${cleanStatus(r.statut)}`
  );
  ...
  const linesUpd = changedRows.map(
    r => `• ${formatLink(cleanHorseNameForDiscord(r.horse), r.horseUrl)} — ${r.date} — ${r.track} — ${formatLink(r.race, r.raceUrl)} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${cleanStatus(r.oldStatut)} → ${cleanStatus(r.statut)}`
  );
