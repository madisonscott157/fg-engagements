...
const cleanHorseNameForSheet = (name) => {
  if (!name) return '';
  // First remove PS. or ps. or P.S. variations (with or without dot before)
  let cleaned = name
    .replace(/\.?PS\./gi, '')
    .replace(/\.?P\.S\./gi, '')
    .trim();
  
  // Remove country codes and everything after
  cleaned = cleaned
  ...
  // Remove trailing sex/age codes like "F. 2 A." or "M. 3 A." or "H. 4 A."
  cleaned = cleaned
    .replace(/\s+[MFH]\.\s*\d+\s*[Aa]\.?\s*$/i, '')
    .trim();
...
async function appendToDoc(docs, entries) {
  ...
  try {
    ...
    // Add hyperlinks with black color and NOT bold
...
(async () => {
  ...
  if (auth && SPREADSHEET_ID) {
    ...
    for (const r of newRows) {
      ...
      const notes = `${r.date} — ${r.track} — ${cleanDoubleParens(r.race)} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${cleanedStatus}`;
    ...
    for (const r of changedRows) {
      ...
      const notes = `${r.date} — ${r.track} — ${cleanDoubleParens(r.race)} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${oldStatus} → ${newStatus}`;
