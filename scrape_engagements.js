...
const cleanHorseNameForSheet = (name) => {
  ...
  // Remove sex/age pattern anywhere: M. 2 A., F 3 a., H. 4 A, etc.
  // This pattern matches: optional space, M/F/H, optional dot, space(s), digit(s), space(s), A, optional dot
  cleaned = cleaned.replace(/\s*[MFH]\.?\s+\d+\s*[Aa]\.?\s*$/i, '');
  
  // Also try without the trailing A (in case format is just "F. 2")
  cleaned = cleaned.replace(/\s*[MFH]\.?\s+\d+\s*$/i, '');
...
async function appendToDoc(docs, entries) {
  ...
  try {
    ...
    // Build formatting requests - ONLY bold the specific ranges we tracked
    const formatRequests = [];
    
    // Apply bold to each tracked range
...
(async () => {
  ...
  if (auth && SPREADSHEET_ID) {
    ...
    for (const r of changedRows) {
      ...
      const notes = cleanDoubleParens(`${r.date} — ${r.track} — ${r.race} (${cleanCategory(r.cat) || '-'}) — ${r.dist || '-'} — Statut: ${oldStatus} → ${newStatus}`);
