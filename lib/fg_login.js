...
async function ensureLoggedIn(page, context, { email, password, targetUrl }) {
  ...
  // Post-OIDC settle. After performCiamLogin we're on
  // /fr/openid-connect/sso?code=... — navigating directly from there to a
  // resource URL (/fr/entraineur/XXX, /fr/professionnels/...) can cause FG
  // to re-trigger SSO and serve "Accès refusé" instead of the page.
  // Explicitly visit /fr/account first so the app-level session is
  // committed before we navigate to the target.
  try {
  ...
  if (targetUrl) {
    ...
    if (!page.url().includes(targetPath)) {
      ...
      await dismissCookieBanner(page);
      // If we ping-pong back to auth (session didn't take), give up loudly.
      if (authRequired(page) || /Accès refusé/i.test(await page.title().catch(() => ''))) {
        throw new Error('Still redirected to auth or access-denied after login for ' + targetUrl);
      }
    }
  }
